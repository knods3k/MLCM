...
class TrainingHandler():
    def __init__(self, model, data_handler, model_file=None, verbosity=0, patience=float('inf')):
        self.model = model
        self.model.to(DEVICE)
        self.data_handler = data_handler
        self.model_file = model_file
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.model.hyperparams.learning_rate)

        self.verbosity = verbosity
        
        self.data_handler.batch_size = self.model.hyperparams.batch_size
        self.test_x, self.test_y = self.data_handler.get_test_data()
        self.test_x.to(DEVICE)
        self.test_y.to(DEVICE)
        self.train_loader = self.data_handler.get()

        if patience is not float('inf'):
            self.early_stopping = EarlyStopping(patience=patience)
        else:
            self.early_stopping = lambda _: None
        
        if verbosity >= 1:
            def print_loss(self, epoch, loss):
                print("Epoch {}/{} Done, Total Loss: {}".format(epoch, self.model.hyperparams.epochs,
                                                                loss / len(self.train_loader)),
                                                                end='\r', flush=True)
        else:
            self.print_loss = lambda _,__: None



    def start_training(self):
        self.avg_losses = torch.zeros(self.model.hyperparams.epochs)
        for epoch in range(self.model.hyperparams.epochs):
            self.model.train()
            self.avg_loss = 0.
            epoch = epoch
            ...
            for x, y in self.train_loader:
                ...
                loss = self.model.hyperparams.criterion(out, y.to(DEVICE))
            ...
            validation_loss = self.model.hyperparams.criterion(
                self.model(self.test_x.to(DEVICE)), self.test_y.to(DEVICE))
            if self.early_stopping(validation_loss):
                break
            self.print_loss(epoch, validation_loss)
            self.avg_losses[self.epoch] = self.avg_loss / len(self.train_loader) 
